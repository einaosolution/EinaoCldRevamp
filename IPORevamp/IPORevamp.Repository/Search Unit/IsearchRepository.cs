 ...
 public    interface IsearchRepository : IAutoDependencyRegister
    {
        ...
        Task<List<IPORevamp.Data.Entity.Interface.Entities.Search.DataResult>> GetFreshMigratedApplication();
        Task<List<IPORevamp.Data.Entity.Interface.Entities.TrademarkType.TrademarkType>> GetTradeMarkType();
        void SaveApplicationHistory(int id, string userrole, HttpRequest request, string tostatus, string toDatastatus, string fromDatastatus, string fromstatus, string comment, string description, string userid,string uploadpath);
