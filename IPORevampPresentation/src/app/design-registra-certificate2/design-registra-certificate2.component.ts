...
@Component({
  ...
  styleUrls: ['./design-registra-certificate2.component.css']
})
export class DesignRegistraCertificate2Component implements OnInit {
...
onChange( deviceValue) {
  ...
  getallApplication2() {
    ...
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignDelegateApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/DesignCertificateExaminePayment"))  {
