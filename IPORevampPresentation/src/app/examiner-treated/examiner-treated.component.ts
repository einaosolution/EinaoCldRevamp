...
@Component({
  selector: 'app-examiner-treated',
  templateUrl: './examiner-treated.component.html',
  styleUrls: ['./examiner-treated.component.css']
})
export class ExaminerTreatedComponent implements OnInit {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetExaminerTreatedApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
 ...
 console.log(response)
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/ExaminerTreated"))  {
