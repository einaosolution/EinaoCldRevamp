...
@Component({
  selector: 'app-pay-patent-certificate',
  templateUrl: './pay-patent-certificate.component.html',
  styleUrls: ['./pay-patent-certificate.component.css']
})
export class PayPatentCertificateComponent implements OnInit {
...
if ( this.row10.length > 0) {
  ...
  localStorage.setItem('pwalletid', this.row10.toString());
  localStorage.setItem('quantity', this.row10.length.toString());


  this.router.navigateByUrl('/Patent/NoticePatentCertificatePayment');
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Patent/PayPatentCertificate"))  {
