...
@Component({
  selector: 'app-designmigrateapplication',
  templateUrl: './designmigrateapplication.component.html',
  styleUrls: ['./designmigrateapplication.component.css']
})
export class DesignmigrateapplicationComponent implements OnInit {
...
this. getallApplication2()
  ...
  onSubmit2(f) {
    ...
    if (!this.appcomment2  ) {
     ...
     if (!this.appdescription) {
      ...
      Swal.fire(
        " Specify Where Application Will Be Moved To ",
        '',
        'error'
      )
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignMigrateApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/DesignTreatMigratedApp"))  {
