...
@Component({
  selector: 'app-back-end-user-profile',
  templateUrl: './back-end-user-profile.component.html',
  styleUrls: ['./back-end-user-profile.component.css']
})
export class BackEndUserProfileComponent implements OnInit {
...
showcountry2() {
 ...
 // this.modalRef = this.modalService.show(ref );
  ...
  getCountry() {
   ...
   var userid = localStorage.getItem('UserId');
   let loggedinemail=localStorage.getItem('username')
   this.busy =   this.registerapi
   .GetEmail(loggedinemail)
   .then((response: any) => {
     this.spinner.hide();
     console.log("Response")
    // this.row3 = response;
    this.row3 = []
    this.row3.push(response)
     console.log(response)
     this.dtTrigger.next();
