...
@Component({
  selector: 'app-patent-paid-certificae',
  templateUrl: './patent-paid-certificae.component.html',
  styleUrls: ['./patent-paid-certificae.component.css']
})
export class PatentPaidCertificaeComponent implements OnInit {
...
.then((response: any) => {
 ...
 // this.router.navigateByUrl('/Dashboard/AcceptanceLetter');
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetPatentPaidCertificate(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Patent/PatentPaidCertificate"))  {
