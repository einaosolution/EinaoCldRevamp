...
@Component({
  selector: 'app-app-status-ds',
  templateUrl: './app-status-ds.component.html',
  styleUrls: ['./app-status-ds.component.css']
})
export class AppStatusDsComponent implements OnDestroy ,OnInit {
  ...
  onSubmit5(emp) {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.value) {
          ...
          this.registerapi
        .DeleteDSApplicationStatus(emp.id,userid)
        .then((response: any) => {
          this.spinner.hide();
          console.log("Response")
...
showcountry2() {
 ...
 // this.modalRef = this.modalService.show(ref );
  ...
  getAllTm() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetAllDSApplicationStatus(userid)
    .then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/AppStatusDs"))  {
