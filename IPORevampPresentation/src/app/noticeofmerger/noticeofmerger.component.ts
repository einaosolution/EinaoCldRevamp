...
@Component({
  selector: 'app-noticeofmerger',
  templateUrl: './noticeofmerger.component.html',
  styleUrls: ['./noticeofmerger.component.css']
})
export class NoticeofmergerComponent implements OnInit {
...
try {
 let f2 = this.fileInput2.nativeElement;
  ...
  try {
    let f2 = this.fileInput4.nativeElement;
      if (f2.files && f2.files[0]) {
       let fileToUpload = f2.files[0];
  ...
  formData.append("Name2",this.account.name2);
  formData.append("Address2",this.account.address2);
  formData.append("Comment",this.account.comment);
  formData.append("Nationality",this.account.nationality);
  formData.append("MergerDate",formatDate(this.account.assignmentdate, 'MM/dd/yyyy', 'en'));
 ...
 // formData.append("nice_class_description",this.account.trademarkdesc);
  ...
  .then((response: any) => {
   ...
   // alert(this.NoticeAppID)
    ...
    this.busy =   this.registerapi
.GetMergerApplicationById(this.NoticeAppID,userid)
.then((response: any) => {
  ...
  this.image2  = response.content.deedOfAssigment
  this.image3  = response.content.certificate

  let obj2 = this.rows.find(o => o.id === response.content.assigneeNationality);

this.country= obj2.name
...
//$(this).steps("next");
  ...
  var vdesc =  localStorage.getItem('description');
    ...
    //    alert("success")
      ...
      this.busy =   this.registerapi
      .GetApplicationById(this.pwalletid,userid)
      .then((response: any) => {
      ...
      this.busy =   this.registerapi
      .GetMergerApplicationByAppId(this.pwalletid,userid)
      .then((response: any) => {
            ...
            buttonsStyling: false,
              ...
              swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                  ...
                  this.account.name = response.content.assignorName
                  this.account.address = response.content.assignorAddress
                  this.account.name2 = response.content.assigneeName
                  this.account.address2 = response.content.assigneeAddress
                  this.account.nationality = response.content.assigneeNationality
                  var mydate = new Date(response.content.dateOfAssignment);
                  this.account.assignmentdate=mydate
                  this.account.comment = response.content.detailOfRequest

                  this.image1  = response.content.powerOfAttorney
                  this.image2  = response.content.deedOfAssigment
                  this.image3  = response.content.certificate

                  this.pwalletid =response.content.applicationid

                  this.NoticeAppID=response.content.id
