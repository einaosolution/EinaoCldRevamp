...
@Component({
  selector: 'app-notice-of-opposition',
  templateUrl: './notice-of-opposition.component.html',
  styleUrls: ['./notice-of-opposition.component.css']
})
export class NoticeOfOppositionComponent implements OnInit {
 ...
 // formData.append("nice_class_description",this.account.trademarkdesc);
  ...
  .then((response: any) => {
   ...
   // alert(this.NoticeAppID)
    ...
    this.busy =   this.registerapi
.GetOpposeFormById(this.NoticeAppID,userid)
.then((response: any) => {
...
//$(this).steps("next");
  ...
  onSubmit3() {
    ...
    this.row = []
    //this.row.push(9)
    this.row.push(parseInt(Fee2.NOTICEOFOPPOSITION))
    var userid = localStorage.getItem('UserId');
