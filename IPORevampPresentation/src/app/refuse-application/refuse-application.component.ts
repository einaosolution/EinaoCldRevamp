...
@Component({
  selector: 'app-refuse-application',
  templateUrl: './refuse-application.component.html',
  styleUrls: ['./refuse-application.component.css']
})
export class RefuseApplicationComponent implements OnInit {
...
onSubmit2() {
  ...
  formData.append("fromstatus","Refused");
  formData.append("tostatus","Registra");
  ...
  this.registerapi
.SendRegistraEmail(userid,this.pwalletid)
.then((response: any) => {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetRefuseApplicationByUserid(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/RefuseApplication"))  {
