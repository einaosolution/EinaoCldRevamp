...
@Component({
  selector: 'app-gen-recordal-change-of-name2',
  templateUrl: './gen-recordal-change-of-name2.component.html',
  styleUrls: ['./gen-recordal-change-of-name2.component.css']
})
export class GenRecordalChangeOfName2Component implements OnInit {
  ...
  this.busy =   this.registerapi.UpdateChangeOfNameStatus(this.pwalletid2,userid,Status.Approved)
.then((response: any) => {
...
onSubmit30() {
  $("#createmodel").modal('hide');
  this.router.navigateByUrl("/Dashboard/Changeofnamecertificate")
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetRecordalChangeOfName(userid ,Status.Approved)
    .then((response: any) => {
...
// .GetApplicationByDocumentId("7",userid)
 .GetChangeOfNameApplicationById(this.pwalletid2,userid2)
 .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/TreatedChangeOfName"))  {
