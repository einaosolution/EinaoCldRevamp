...
@Component({
  selector: 'app-noticeofchangeofaddress',
  templateUrl: './noticeofchangeofaddress.component.html',
  styleUrls: ['./noticeofchangeofaddress.component.css']
})
export class NoticeofchangeofaddressComponent implements OnInit {
...
if (f) {
  ...
  formData.append("oldaddress",this.account.address);
  formData.append("newaddress",this.account.address2);
  formData.append("requestdetail",this.account.detailrequest);
 ...
 // formData.append("nice_class_description",this.account.trademarkdesc);
  ...
  .then((response: any) => {
   ...
   // alert(this.NoticeAppID)
    ...
    this.busy =   this.registerapi
.GetChangeOfAddressApplicationById(this.NoticeAppID,userid)
.then((response: any) => {
...
//$(this).steps("next");
  ...
  localStorage.setItem('PaymentType',"changeofaddress");
