...
@Component({
  selector: 'app-pay-certificate',
  templateUrl: './pay-certificate.component.html',
  styleUrls: ['./pay-certificate.component.css']
})
export class PayCertificateComponent implements OnInit {
...
if ( this.row10.length > 0) {
  ...
  this.router.navigateByUrl('/Dashboard/NoticeCertificatePayment');
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetCertificateFreshApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/PayCertificate"))  {
