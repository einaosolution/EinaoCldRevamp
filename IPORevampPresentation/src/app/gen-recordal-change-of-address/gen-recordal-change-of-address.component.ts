...
@Component({
  selector: 'app-gen-recordal-change-of-address',
  templateUrl: './gen-recordal-change-of-address.component.html',
  styleUrls: ['./gen-recordal-change-of-address.component.css']
})
export class GenRecordalChangeOfAddressComponent implements OnInit {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetRecordalChangeOfAddress(userid,Status.Paid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/ChangeOfAddressBackend"))  {
