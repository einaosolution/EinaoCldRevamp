...
@Component({
  selector: 'app-notice-certificate-payment',
  templateUrl: './notice-certificate-payment.component.html',
  styleUrls: ['./notice-certificate-payment.component.css']
})
export class NoticeCertificatePaymentComponent implements OnInit {
...
//$(this).steps("next");
  ...
  onSubmit3() {
   ...
   // let qty = parseInt(this.account.quantity)
    ...
    for (let i = 0; i < qty; i++) {
    //  this.row.push(13)
    this.row.push(parseInt(Fee2.APPLICATIONFORISSUANCEOFCERTIFICATEFORTRADESERVICE))
    }
  ...
  var vdesc =  localStorage.getItem('description');
    ...
    if (firstParam) {
      ...
      this.busy =   this.registerapi
      .GetApplicationById2(this.pwalletid,userid)
      .then((response: any) => {
