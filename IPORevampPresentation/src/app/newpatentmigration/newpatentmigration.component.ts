...
@Component({
  selector: 'app-newpatentmigration',
  templateUrl: './newpatentmigration.component.html',
  styleUrls: ['./newpatentmigration.component.css']
})
export class NewpatentmigrationComponent implements OnInit {
...
console.log(this.row101)
  ...
  formData.append("registration_number",this.userform.value.patentregistration);
  formData.append("filingdate",formatDate(this.userform.value.filingdate, 'MM/dd/yyyy', 'en'));
...
onChange3(deviceValue) {
  ...
  this.registerapi
.PatentcountByRegistration(this.userform.value.patentregistration )
.then((response: any) => {
...
if (parseInt(response) > 0) {
  (<FormControl> this.userform.controls['patentregistration']).setValue("");
...
this.userform3.reset()
 ...
 // (<FormControl> this.userform.controls['Description']).setValue("");
  ...
  //  this.dtTrigger.next();
    ...
    this.registerapi.setPage("patent")

    this.registerapi.VChangeEvent("patent");
    ...
    this.userform = this.formBuilder.group({
      ...
      patentregistration:  [null,Validators.required ],
