...
@Component({
  selector: 'app-searchmigrateapp',
  templateUrl: './searchmigrateapp.component.html',
  styleUrls: ['./searchmigrateapp.component.css']
})
export class SearchmigrateappComponent implements OnInit {
...
onChange( deviceValue) {
  ...
  onSubmit2(f) {
   ...
   formData.append("fromstatus",Status.Pending);
   formData.append("tostatus",Status.Fresh);
   formData.append("fromDatastatus",DataStatus.Migration);
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetFreshMigratedApplication(userid)
    .then((response: any) => {
...
let test = this.fileInput.nativeElement;
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/TreatMigratedApp"))  {
