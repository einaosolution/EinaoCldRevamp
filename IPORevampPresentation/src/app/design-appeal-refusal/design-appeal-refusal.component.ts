...
@Component({
  selector: 'app-design-appeal-refusal',
  templateUrl: './design-appeal-refusal.component.html',
  styleUrls: ['./design-appeal-refusal.component.css']
})
export class DesignAppealRefusalComponent implements OnInit {
...
onSubmit2() {
  ...
  this.registerapi
.SendRegistraDesignAppealEmail(userid,this.pwalletid)
.then((response: any) => {
 ...
 // this.router.navigateByUrl('/Dashboard/AcceptanceLetter');
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignRefuseApplication(userid)
    .then((response: any) => {
  ...
  showdetail2(kk) {
    ...
    this.busy =   this.registerapi
    .GetDesignRefuseDate( this.pwalletid )
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/DesignAppealRefusal"))  {
