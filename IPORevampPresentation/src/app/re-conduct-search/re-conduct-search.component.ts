...
@Component({
  selector: 'app-re-conduct-search',
  templateUrl: './re-conduct-search.component.html',
  styleUrls: ['./re-conduct-search.component.css']
})
export class ReConductSearchComponent implements OnInit {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetExaminerReconductSearch(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/ReConductSearch"))  {
    ...
    this.registerapi.setPage("Re-conduct Search")

    this.registerapi.VChangeEvent("Re-conduct Search");
