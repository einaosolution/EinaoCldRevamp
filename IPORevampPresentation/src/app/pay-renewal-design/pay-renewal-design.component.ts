...
@Component({
  selector: 'app-pay-renewal-design',
  templateUrl: './pay-renewal-design.component.html',
  styleUrls: ['./pay-renewal-design.component.css']
})
export class PayRenewalDesignComponent implements OnInit {
...
//$(this).steps("next");
  ...
  localStorage.setItem('PaymentType',"PayDesignRenewal");
  ...
  ngOnInit() {
   ...
   let feedesc= ""
   if (designtype =="Textile") {
    feedesc ="RENEWAL OF TEXTILE DESIGN CERTIFICATE"
   }

   else {
    feedesc ="RENEWAL OF NON-TEXTILE DESIGN"
   }
  ...
  var vdesc =  localStorage.getItem('description');
    ...
    if (firstParam) {
      ...
      this.busy =   this.registerapi
      .GetRenewalDesignApplicationById(userid,this.pwalletid)
      .then((response: any) => {
...
if (Settings.itemValue =="0"    ) {
  ...
  this.busy =   this.registerapi
.GetFeeListByName(feedesc,userid)
.then((response: any) => {
