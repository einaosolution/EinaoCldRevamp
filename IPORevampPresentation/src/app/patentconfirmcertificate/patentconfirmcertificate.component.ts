...
@Component({
  selector: 'app-patentconfirmcertificate',
  templateUrl: './patentconfirmcertificate.component.html',
  styleUrls: ['./patentconfirmcertificate.component.css']
})
export class PatentconfirmcertificateComponent implements OnInit {
  ...
  this.router.navigateByUrl("/Patent/PatentCertificate")
...
.then((response: any) => {
 ...
 // this.router.navigateByUrl('/Dashboard/AcceptanceLetter');
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetPatentConfirmedCertificate(userid)
    .then((response: any) => {
...
this.pwalletid = kk.applicationId
   ...
   localStorage.setItem("Applicationid",this.pwalletid);
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Patent/Patentconfirmcertificate"))  {
    ...
    this.registerapi.setPage("PatentCertificate")

    this.registerapi.VChangeEvent("PatentCertificate");
