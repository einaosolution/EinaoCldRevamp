...
@Component({
  selector: 'app-pay-renewal-patent',
  templateUrl: './pay-renewal-patent.component.html',
  styleUrls: ['./pay-renewal-patent.component.css']
})
export class PayRenewalPatentComponent implements OnInit {
 ...
 formData.append("NextRenewal",this.nextrenewal);
  ...
  .then((response: any) => {
   ...
   // alert(this.NoticeAppID)
    ...
    this.busy =   this.registerapi
.GetRenewalPatentApplicationById(this.NoticeAppID,userid)
.then((response: any) => {
...
//$(this).steps("next");
  ...
  localStorage.setItem('PaymentType',"PayPatentRenewal");
  ...
  onSubmit3() {
    ...
    this.row = []
    let designtype =localStorage.getItem('designtype');
    this.row.push(parseInt(Fee2.ANNUALPATENTRENEWAL))
  ...
  var vdesc =  localStorage.getItem('description');
    ...
    if (firstParam) {
      ...
      this.busy =   this.registerapi
      .GetPatentApplicationById4(userid,this.pwalletid)
      .then((response: any) => {

        this.row10 = response.content;
        console.log("Applicationbyid response")

       this.account.name = response.content[0].applicantName

      this.account.address= response.content[0].applicantAddress
      this.account.trademarktitle =response.content[0].productTitle

      this.nextrenewal = response.content[0].nextrenewalDate
        console.log(response)
...
this.feelist(Fee2.ANNUALPATENTRENEWAL)
