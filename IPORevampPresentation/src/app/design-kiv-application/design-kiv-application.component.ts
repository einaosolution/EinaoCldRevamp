...
@Component({
  selector: 'app-design-kiv-application',
  templateUrl: './design-kiv-application.component.html',
  styleUrls: ['./design-kiv-application.component.css']
})
export class DesignKivApplicationComponent implements OnInit {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignKivExaminerApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
 ...
 let  userid = localStorage.getItem('UserId');
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/ExaminerKiv"))  {
    ...
    this.registerapi.setPage("DesignExaminer")

    this.registerapi.VChangeEvent("DesignExaminer");
