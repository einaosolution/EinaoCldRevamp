...
@Component({
  selector: 'app-opposition-fresh',
  templateUrl: './opposition-fresh.component.html',
  styleUrls: ['./opposition-fresh.component.css']
})
export class OppositionFreshComponent implements OnInit {
...
onSubmit2() {
  ...
  if (this.appcomment =="") {
   ...
   formData.append("tostatus","Applicant");
  ...
  this.registerapi
  .OppositionSendUserEmail2(userid,this.userid,this.appcomment)
  .then((response: any) => {
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetOppositionFreshApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/OppositionFresh"))  {
    ...
    this.registerapi.setPage("TrademarkOpposition")

    this.registerapi.VChangeEvent("TrademarkOpposition");
  ...
  this.dtOptions = {
   ...
   this.busy =   this.registerapi
    ...
    .then((response: any) => {
      ...
      console.log("opposition Response")
      this.rows = response.content;
      console.log(response)
