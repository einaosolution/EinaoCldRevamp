...
@Component({
  selector: 'app-view-judgment',
  templateUrl: './view-judgment.component.html',
  styleUrls: ['./view-judgment.component.css']
})
export class ViewJudgmentComponent implements OnInit {
...
onSubmit2() {
  ...
  if (this.appcomment =="") {
   ...
   formData.append("fromstatus","Judgement");
...
showcountry2() {
  ...
  getallApplication() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetNewJudgment(userid)
    .then((response: any) => {

      console.log("judgement Response")
      this.rows = response.content;
      console.log(response)
      this.dtTrigger.next();
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/ViewJudgment"))  {
