...
@Component({
  selector: 'app-design-paid-certificate2',
  templateUrl: './design-paid-certificate2.component.html',
  styleUrls: ['./design-paid-certificate2.component.css']
})
export class DesignPaidCertificate2Component implements OnInit {
  ...
  this.router.navigateByUrl("/Design/DesignCertificate")
...
onSubmit2() {
  ...
  this.registerapi
.DesignCertificateSendUserEmail(userid,this.pwalletid)
.then((response: any) => {
 ...
 // this.router.navigateByUrl('/Dashboard/AcceptanceLetter');
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignCertificateConfirmapplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/DesignConfirmCertificate"))  {
    ...
    this.registerapi.setPage("DesignCertificate")

    this.registerapi.VChangeEvent("DesignCertificate");
