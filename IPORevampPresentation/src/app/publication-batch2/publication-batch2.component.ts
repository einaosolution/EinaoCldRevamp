...
@Component({
  selector: 'app-publication-batch2',
  templateUrl: './publication-batch2.component.html',
  styleUrls: ['./publication-batch2.component.css']
})
export class PublicationBatch2Component implements OnInit {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/PublicationBatch"))  {
  ...
  this.dtOptions = {
   ...
   this.busy =   this.registerapi
    .GetDesignBatches(userid)
    .then((response: any) => {

      console.log("Design Batch Response")
      this.rows = response.content;
      console.log(response)
