...
export class UnitsComponent implements OnDestroy ,OnInit {
  ...
  onSubmit() {
    ...
    if (obj.departmentId == this.userform.value.Department) {
...
showcountry(kk) {
  ...
  (<FormControl> this.userform.controls['Department']).setValue(kk.departmentId);
...
showcountry2() {
 ...
 // this.modalRef = this.modalService.show(ref );
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Dashboard/Unit"))  {
