...
@Component({
  selector: 'app-patentmigrationappliction',
  templateUrl: './patentmigrationappliction.component.html',
  styleUrls: ['./patentmigrationappliction.component.css']
})
export class PatentmigrationapplictionComponent implements OnInit {
...
this. getallApplication2()
  ...
  onSubmit2(f) {
    ...
    if (!this.appcomment2  ) {
     ...
     if (!this.appdescription) {
      ...
      Swal.fire(
        " Select where application will be moved to",
        '',
        'error'
      )
   ...
   formData.append("fromstatus",this.Status.Pending);
   formData.append("tostatus",this.Status.Fresh);
   formData.append("fromDatastatus",this.DataStatus.Migration);
   formData.append("toDatastatus",this.appdescription);
   formData.append("userid",userid);
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetPatentMigrateApplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Patent/PatentTreatMigratedApp"))  {
    ...
    this.registerapi.setPage("MigratedData")

    this.registerapi.VChangeEvent("MigratedData");
