...
@Component({
  selector: 'app-design-paid-certificate-component',
  templateUrl: './design-paid-certificate-component.component.html',
  styleUrls: ['./design-paid-certificate-component.component.css']
})
export class DesignPaidCertificateComponentComponent implements OnInit {
...
.then((response: any) => {
 ...
 // this.router.navigateByUrl('/Dashboard/AcceptanceLetter');
  ...
  getallApplication2() {
    var userid = localStorage.getItem('UserId');
    this.busy =   this.registerapi
    .GetDesignCertificateFreshapplication(userid)
    .then((response: any) => {
...
.then((response: any) => {
  ...
  ngOnInit() {
    ...
    if (this.registerapi.checkAccess("#/Design/DesignVerifyPaidCertificate"))  {
